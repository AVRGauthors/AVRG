...
class BaseVRG(abc.ABC):
    ...
    Base class for VRG
    """
    __slots__ = ('name', 'extract_type', 'clustering', 'mu', 'rule_list', 'rule_dict', 'cost', 'num_rules', 'unique_rule_list',
                 'unique_rule_dict', 'unique_rule_rhs')

    def __init__(self, extract_type: str, clustering: str, name: str, mu: int, kind: str = 'VRG'):
        self.name: str = name  # name of the graph
        self.extract_type: str = extract_type  # type of grammar - mu, local, global, selection strategy - random, dl, level, or dl_levels
        self.kind = kind  # VRG/A-VRG/NCE
    ...
    def copy(self):
        vrg_copy = BaseVRG(extract_type=self.extract_type, clustering=self.clustering, name=self.name, mu=self.mu)
    ...
    def __str__(self):
        if self.cost == 0:
            self.calculate_cost()
        st = f'graph: {self.name!r}, mu: {self.mu}, extract type: {self.extract_type!r} clustering: {self.clustering!r} rules: {len(self.rule_list):_d}' \
...
class VRG(BaseVRG):
    ...
    def copy(self):
        vrg_copy = VRG(type=self.extract_type, clustering=self.clustering, name=self.name, mu=self.mu)
        vrg_copy.rule_list = self.rule_list[:]
        vrg_copy.rule_dict = dict(self.rule_dict)
        vrg_copy.cost = self.cost
        vrg_copy.num_rules = self.num_rules
...
class AttributedVRG(VRG):
    def __init__(self, clustering: str, name: str, mu: int, attr_name: str, extract_type: str):
        super().__init__(kind='A-VRG', clustering=clustering, name=name, mu=mu, extract_type=extract_type)
        self.attr_name = attr_name


class AVRGLink(AttributedVRG):
    """
    Link pred requires us to store ALL rules separately, skipping isomorphism check
    """
    def __init__(self, clustering: str, name: str, mu: int, attr_name: str, extract_type: str):
        super().__init__(attr_name='value', clustering=clustering, name=name, mu=mu, extract_type=extract_type)
...
class NCE:
    ...
    """
    __slots__ = 'name', 'kind', 'extract_type', 'clustering', 'mu', 'rule_list', 'rule_dict',\
                'unique_rule_list', 'unique_rule_dict', 'unique_rule_rhs', 'cost', 'num_rules'

    def __init__(self, clustering: str, name: str, mu: int, extract_type: str):
        self.name: str = name  # name of the graph
        self.kind: str = 'NCE'
        self.clustering: str = clustering  # clustering strategy
        self.mu = mu
    ...
    def __str__(self):
        if self.cost == 0:
            self.calculate_cost()
        st = f'graph: {self.name}, mu: {self.mu}, kind: {self.kind} extract: {self.extract_type} clustering: {self.clustering} ' \
    ...
    def add_rule(self, rule: NCERule) -> int:
