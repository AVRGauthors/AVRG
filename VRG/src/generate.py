...
class AttributedEnsureAllNodesGenerator(AttributedRandomGenerator):
    ...
    def _gen(self) -> LightMultiGraph:
        _gen_graph = super()._gen()
        # rename the nodes based on actual label
        renamed_g = nx.relabel_nodes(_gen_graph, mapping=nx.get_node_attributes(_gen_graph, 'actual_label'))
        return renamed_g
