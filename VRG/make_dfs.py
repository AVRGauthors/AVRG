...
from VRG.src.utils import load_pickle, nx_to_igraph, check_file_exists, dump_pickle
...
def make_graph_df_new(name: str, fname: str, basedir: str, orig_gstats: GraphStats, slow_stats: bool,
    ...
    for g in gen_graphs:
        ...
        row = dict(name=name, orig_graph=np.nan, model=model, gen_type=gen_type, extract_type=extract_type,
        ...
        if slow_stats:
            ...
            row.update(dict(orig_apl=orig_apl, orig_avg_cc=orig_avg_cc, gen_apl=gen_apl, gen_avg_cc=gen_avg_cc))
    ...
    for name in names:
        ...
        for fname in tqdm(files, total=len(files), desc=f'{name}'):
            ...
            row = dict(name=name, orig_n=orig_graph.order(), orig_m=orig_graph.size(), grammar_type=grammar_type,
                       extract_type=vrg.extract_type, mu=int(mu), clustering=clustering, cost=dc, branch_factor=avg_branch_factor,
...
def make_all_graph_dfs_new(basedir, name, clusterings, models=None, final=False, slow_stats=False, mus=None, extract_types=None):
    dfs = []
    ...
    for model in models:
        ...
        for fname in tqdm(glob.glob(f'{basedir}/output/graphs/{name}/{model}*'), desc=f'{name}', ncols=100):
            path = Path(fname)
            ...
            if model == 'AVRG':
                ...
                if existing_df is not None:
                    if not existing_df[(existing_df.name==name) & (existing_df.model==model) &
            ...
            df = make_graph_df_new(basedir=basedir, fname=fname, name=name, orig_gstats=orig_gstats, model=model,
...
def get_final_graph_dfs():
    ...
    mus = [5]
    # models = ['AVRG', 'NetGAN', 'CELL', 'CL', 'AGM'] #, 'linear_ae', 'linear_vae', 'gcn_ae', 'gcn_vae']
    models = ['gcn_ae', 'gcn_vae']
    args = []
    final, slow_stats = True, False
    # basedir, name, clusterings, models=None, final=False, slow_stats=False, mus=None, extract_types=None
    for name in names:
        args.append((basedir, name, clusterings, models, final, slow_stats, mus, extract_types))

    parallel_async(func=make_all_graph_dfs_new, args=args, num_workers=5)
    # make_all_graph_dfs_new(basedir=basedir, name=name, clusterings=clusterings, mus=mus, models=models,
